...
func help() {
	...
	fmt.Println("  nvm current, c                  : Display active version.")
	fmt.Println("  nvm debug                       : Check the NVM4W process for known problems ")
	fmt.Println("                                    (troubleshooter).")
	fmt.Println("  nvm install, i <version> [arch] : The version can be a specific version, \"latest\"")
	fmt.Println("                                    for the latest current version, or \"lts\" for ")
	fmt.Println("                                    the most recent LTS version. Optionally specify")
	fmt.Println("                                    whether to install the 32 or 64 bit version ")
	fmt.Println("                                    (defaults to system arch). Set [arch] to \"all\" ")
	fmt.Println("                                    to install 32 AND 64 bit versions. Add --insecure ")
	fmt.Println("                                    to the end of this command to bypass SSL ")
	fmt.Println("                                    validation of the remote download server.")
	fmt.Println("  nvm list, l [available]         : List the node.js installations. Type \"available\"")
	fmt.Println("                                    at the end to see what can be installed.")
	fmt.Println("  nvm on                          : Enable node.js version management.")
	fmt.Println("  nvm off                         : Disable node.js version management.")
	fmt.Println("  nvm proxy [url]                 : Set a proxy to use for downloads. Leave [url]")
	fmt.Println("                                    blank to see the current proxy. Set [url] to")
	fmt.Println("                                    \"none\" to remove the proxy.")
	fmt.Println("  nvm node_mirror [url]           : Set the node mirror. Defaults to ")
	fmt.Println("                                    https://nodejs.org/dist/.")
	fmt.Println("                                    Leave [url] blank to use default url.")
	fmt.Println("  nvm npm_mirror [url]            : Set the npm mirror. Defaults to ")
	fmt.Println("                                    https://github.com/npm/cli/archive/. ")
	fmt.Println("                                    Leave [url] blank to default url.")
	...
	fmt.Println("  nvm use, u [version] [arch]     : Switch to use the specified version. Optionally ")
	fmt.Println("                                    use \"latest\", \"lts\", or \"newest\".")
	fmt.Println("                                    \"newest\" is the latest installed version. ")
	fmt.Println("                                    Optionally specify 32/64bit architecture.")
	fmt.Println("                                    nvm use <arch> will continue using the selected ")
	fmt.Println("                                    version, but switch to 32/64 bit mode.")
	fmt.Println("  nvm root [path]                 : Set the directory where nvm should store ")
	fmt.Println("                                    different versions of node.js. If <path> ")
	fmt.Println("                                    is not set, the current root will be displayed.")
	fmt.Println("  nvm [--]version                 : Displays the current running version of nvm")
	fmt.Println("                                    for Windows. Aliased as v.")
	fmt.Println(" ")
}
...
func getLTS() string {
	...
	// Code never executes, it's just for skipping unused warning
	// I'll improve it
	if false {
		fmt.Println(all)
		fmt.Println(ltsList)
