...
// alias assigns a friendly name to the version installed. This will use the format of 'alias|version' => 'LTS|12.14.1' in settings.
func alias(args []string) {
	// no params so just show what aliases we currently have
	if len(args) == 2 {
		fmt.Println("\nCurrent aliases:")
		fmt.Println("----------------")

		for i, v := range env.aliases {
			fmt.Println(i + " => " + v)
		}
	...
	// Removing the alias since version was not provided
	if len(args) == 3 {
		if _, ok := env.aliases[name]; !ok {
			fmt.Println("Alias '" + name + "' doesn't exist")
			return
		}

		delete(env.aliases, name)
		saveSettings()
		fmt.Println("Removed alias " + name)
	...
	fmt.Println("Associating alias " + name + " => " + version)

	env.aliases[name] = version
...
func use(version string, cpuarch string) {
	...
	// add extra check for using an alias
	if v, ok := env.aliases[version]; ok {
		fmt.Println("Using alias " + version + " => " + v)
		version = v
	}
...
func help() {
	...
	fmt.Println("  nvm alias [name] [version]   : Creates a friendly alias to version. If no version is provided the alias will be removed.")
	fmt.Println(" ")
}
...
func saveSettings() {
	...
	aliases, err := json.Marshal(&env.aliases)
	if err != nil {
		fmt.Println("Unable to save aliases: " + fmt.Sprint(err))
	}
...
func setup() {
	...
	for _, line := range lines {
		...
		if strings.HasPrefix(line, "root:") {
			...
			aliases := strings.TrimSpace(regexp.MustCompile("^aliases:").ReplaceAllString(line, ""))

			if aliases != "" {
				err := json.Unmarshal([]byte(aliases), &env.aliases)
				if err != nil {
					fmt.Println("Unable to parses aliases: " + fmt.Sprint(err))
				}
			}
		}
	}
